<template>
  <table class="table table-xl table-hover table-group-divider">
    ...
    <tbody>
      <tr v-for="info in tabDatas">
        ...
        <td>{{ info.Internships[0].name }} </td>
        <td :id="info.id"><img @click="deleting($event)" src="../assets/delete.png" /> <img @click="updating($event)"
            src="../assets/update.png" /></td>
      </tr>
    </tbody>
  </table>
  ...
  <div class="card mainCard" v-if="showCard">
    <div class="card-body">
      ...
      <div class="input-group mb-3">
        <input v-model="toSend.parentName" type="text" class="form-control" placeholder="Nom du parent"
          aria-describedby="basic-addon1" />
      </div>

      <div class="input-group mb-3">
        <input v-model="toSend.parentPhone" type="text" placeholder="Téléphone" class="form-control"
          aria-describedby="basic-addon1" />
      </div>

      <div class="input-group mb-3">
        <input v-model="toSend.mail" type="email" placeholder="Email" class="form-control"
          aria-describedby="basic-addon1" />
      </div>
      ...
      <div class="input-group mb-3">
        <input v-model="toSend.childName" type="text" class="form-control" placeholder="Nom de l'enfant"
          aria-describedby="basic-addon1" />
      </div>

      <div class="input-group mb-3">
        <input v-model="toSend.age" type="number" class="form-control" placeholder="Age"
          aria-describedby="basic-addon1" />
      </div>
      <div class="input-group mb-3">
        <input v-model="toSend.healthIssues" type="text" class="form-control" placeholder="Problèmes de santé?"
          aria-describedby="basic-addon1" />
      </div>
      <div>
      ...
      <div class="input-group mb-1">
        ...
        <select class="custom-select my-1 mr-sm-2" id="inlineFormCustomSelectPref">
          <option selected>Choisir</option>
          <option v-for="info in tabToSelect" :value=info> {{ info }}</option>
        </select>
  ...
  <div class="card mainCard" v-if="showCardUp">
    <div class="card-body">
      ...
      <div class="input-group mb-3">
        <input v-model="toSend.paid" type="text" class="form-control" placeholder="Payé?"
          aria-describedby="basic-addon1" />
      </div>
      ...
      <div class="input-group mb-3">
        <input v-model="toSend.imgRights" type="text" class="form-control" placeholder="Droits à l'image"
          aria-describedby="basic-addon1" />
      </div>
      <div class="input-group mb-3">
        <input v-model="nameShip" type="text" class="form-control" placeholder="Stage de l'enfant"
          aria-describedby="basic-addon1" />
      </div>
  ...
  mounted() {
    ...
    internshipService.getAllInternships().then((datas) => {
      datas.data.forEach((subData) => {
        this.tabToSelect.push(subData.name)
      })
    })
  },
